...
//--------------------------------------------------------------
//
//  Cosmic pions generator
//
//  Author: P.Chimenti
//
//  14-9-2010 v0.1, 26-06-2013 v0.9 
//
//--------------------------------------------------------------
//--------------------------------------------------------------
...
void pions(int INI_RAND=9999,int MAXEVENTS=10000, TString file="event.hepev")
{
  ...
  const Double_t pionMass = 0.13957;//units: GeV/c^2
  ...
  const double pionFraction=0.03;

  double yFluxLog[nbins+1];
  double yFlux[nbins+1];
  for(int i=0; i<nbins; i++){
    yFluxLog[i]=((yPoints[i]-iniPy)/(finPy-iniPy))*(finFy-iniFy)+iniFy;
    yFlux[i]=pow(10,yFluxLog[i])*pionFraction*(xMomentum[i]+xMomentum[i+1])/2.;
  }

  // now set histo
  TH1F* histoFlux =new TH1F ("PionFlux","PionFlux",nbins,xMomentum);
  for(int i=0; i<nbins; i++){
...
//   histoFlux->Draw();
  ...
  TVector3  position;
  Double_t  momentum, ltheta, phi, initX,initY,initZ, pionEnergy;
  ...
  for(;ievent<MAXEVENTS; ievent++){
    ...
    // energy
    pionEnergy = sqrt(momentum*momentum+pionMass*pionMass);
    
    double sign = gRandom->Uniform(-1.,1.);
    if(sign>0) outfile << 1 << " " << 211  << " ";
    else outfile << 1 << " " << -211  << " ";
    outfile    << 0  << " " << 0  << " "//
	    << 0  << " " << 0  << " "//
	    << momentum3d.X()  << " " << momentum3d.Y()  << " " //
	    << momentum3d.Z()  << " " << pionEnergy  << " " //
	    << pionMass  << " " << position.X()*1000  << " " // position in mm
