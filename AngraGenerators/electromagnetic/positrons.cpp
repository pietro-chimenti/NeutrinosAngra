...
void positrons(int INI_RAND=9999,int MAXEVENTS=10000, TString file="event.hepev")
{
  ...
  // now set histo
  TH1F* histoFlux =new TH1F ("PositronFlux","PositronFlux",nbins,xMomentum);
  for(int i=0; i<nbins; i++){
...
//   histoFlux->Draw();
  ...
  int ievent=0;
    ...
    TVector3  position;
    Double_t  momentum, ltheta, phi, initX,initY,initZ, positronEnergy;
  ...
  for(;ievent<MAXEVENTS; ievent++){
   ...
   // energy
    positronEnergy = sqrt(momentum*momentum+electronMass*electronMass);
